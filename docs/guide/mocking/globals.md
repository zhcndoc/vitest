...
你可以使用 [`vi.stubGlobal`](/api/vi#vi-stubglobal) 辅助函数来模拟 `jsdom` 或 `node` 中不存在的全局变量。它会将全局变量的值放入 `globalThis` 对象中。
...
// 现在你可以作为 `IntersectionObserver` 或 `window.IntersectionObserver` 访问它
```
