...
- 明确要求边缘情况。“包含空输入、边界值和错误处理的测试”比把它交给 AI 自己判断能产生更全面的覆盖。没有这个提示，大多数工具只会生成少量快乐路径测试，然后就停下来。
- 如果你希望使用特定的 Vitest 功能，请明确提及。“对错误消息使用 `toMatchInlineSnapshot`”或“对不同货币格式使用 `test.for`”会引导 AI 使用合适的工具，而不是退回到重复的复制粘贴式测试。
- 如果你在测试异步代码，请明确说明。“该函数返回一个 Promise”或“这里会调用外部 API”有助于 AI 使用 `async`/`await` 以及合适的匹配器，如 `.resolves` 和 `.rejects`。
- 告诉 AI 不要做什么。“基于真实实现进行测试，不要 Mock 任何模块”或“不要使用快照测试”可以阻止常见的默认行为。AI 工具往往过度 Mock，而明确的约束可以避免这一点。
- 描述你想要的测试结构。“按方法使用 `describe` 块分组测试”或“用 `test.extend` 夹具处理数据库连接，而不是 `beforeEach`”可以让你省去事后重构输出的麻烦。
- 在请求新增内容时引用现有测试。“遵循 `auth.test.js` 中测试的相同风格”比从头描述风格更有效。AI 会从示例中捕捉命名约定、断言模式和导入风格。
- 如果第一次结果不理想，就迭代。“这些测试过于关注实现细节。请重写它们，只断言返回值和抛出的错误”是一个有效的后续请求。通过对话逐步优化，往往比一开始就写出完美提示能得到更好的结果。
