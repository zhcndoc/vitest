...
::: warning Vitest 5 中已更新
基准测试 API 已被重写。`bench` 不再是从 `vitest` 顶层导入的内容，`bench.skip` / `bench.only` / `bench.todo` 辅助方法已被移除。`bench` 现在是一个从 `test()` 内部访问的 [测试上下文字段](/guide/test-context#bench)。

有关新 API，请参见 [基准测试指南](/guide/benchmarking)。
:::
